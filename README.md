...
| LP_BOT_PRICING_SOURCE | Pricing source: "black_scholes" or "order_book" | black_scholes |
...
| --pricing-source | Mid-price source: "black_scholes" or "order_book" |
...
The bot supports two pricing modes via `pricing_source`:

- **`black_scholes`** (default): Uses Black-Scholes fair value from the underlying data stream as the reference price. Recommended when there are few market participants, as it prevents the bot from anchoring to its own stale orders.
- **`order_book`**: Uses the live order book mid-price. Better for active markets with multiple independent participants.
